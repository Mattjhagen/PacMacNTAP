...
function getSupabaseAdmin() {
  if (supabaseAdmin !== void 0) return supabaseAdmin;
...
function hashPassword(password, salt = crypto2.randomBytes(16).toString("hex")) {
  const passwordHash = crypto2.pbkdf2Sync(password, salt, 12e4, 32, "sha256").toString("hex");
  return { passwordHash, passwordSalt: salt };
}
function verifyPassword(password, user) {
  const candidate = hashPassword(password, user.passwordSalt).passwordHash;
  return crypto2.timingSafeEqual(Buffer.from(candidate, "hex"), Buffer.from(user.passwordHash, "hex"));
}
...
function customerFromRows(customer, line, sim, cycleRow) {
  const cycle2 = cycleRow || currentCycle();
  return {
    id: customer.id,
    name: customer.name,
    ...
    billingCycle: {
      id: cycle2.id || `cycle_${customer.id}_current`,
      startsAt: cycle2.starts_at || cycle2.startsAt,
      endsAt: cycle2.ends_at || cycle2.endsAt
    },
    line: {
...
function fraudAlertFromRow(row) {
  return {
    id: row.id,
    customerId: row.customer_id,
...
function blockedNumberFromRow(row) {
  return {
    id: row.id,
    customerId: row.customer_id,
...
function createJwt(user) {
  ...
  const signature = base64Url(crypto2.createHmac("sha256", JWT_SECRET).update(`${header}.${payload}`).digest());
...
function verifyJwt(token) {
  ...
  const expected = base64Url(crypto2.createHmac("sha256", JWT_SECRET).update(`${header}.${payload}`).digest());
  if (!crypto2.timingSafeEqual(Buffer.from(signature), Buffer.from(expected))) return null;
...
async function login(email, password) {
  const supabase = getSupabaseAdmin();
  if (supabase) {
    const { data: row, error } = await supabase.from("pacmac_app_users").select("*").eq("email", email.toLowerCase()).maybeSingle();
    if (error) {
      ...
      if (!verifyPassword(password, user2)) return { ok: false, error: "Invalid email or password." };
      const safeUser2 = publicUser(user2);
      return { ok: true, user: safeUser2, token: createJwt(safeUser2) };
    }
  }
...
async function signup(input) {
  ...
  const supabase = getSupabaseAdmin();
  if (supabase) {
    const { data: existing, error: existingError } = await supabase.from("pacmac_app_users").select("id").eq("email", email).maybeSingle();
    if (existingError) {
      ...
      const password2 = hashPassword(input.password);
      const customerId2 = crypto2.randomUUID();
      const lineId = crypto2.randomUUID();
      const simId = crypto2.randomUUID();
      const cycleId = crypto2.randomUUID();
      const userId2 = crypto2.randomUUID();
      const phoneNumber = input.phoneNumber || "+1 555 555 0101";
      const cycle2 = currentCycle();
      ...
      const { error: simError } = await supabase.from("sim_cards").insert({
        ...
        eid: `89049032${Math.floor(Math.random() * 1e10)}`,
        sim_type: "eSIM",
        status: "provisioned",
        qr_code_url: `pacmac-esim://${customerId2}`
      });
      if (simError) return { ok: false, error: simError.message };
      const { error: cycleError } = await supabase.from("billing_cycles").insert({
        id: cycleId,
        customer_id: customerId2,
        starts_at: cycle2.startsAt,
        ends_at: cycle2.endsAt,
        status: "open"
      });
      ...
      const safeUser2 = { id: userId2, email, name: input.name, role: "customer", customerId: customerId2 };
      return { ok: true, user: safeUser2, token: createJwt(safeUser2) };
    }
  }
  ...
  const customerId = `cus_${crypto2.randomUUID()}`;
...
async function getSupabaseCustomerDashboard(user) {
  const supabase = getSupabaseAdmin();
  if (!supabase || !user.customerId || !isUuid(user.customerId)) return null;
  const { data: customer, error: customerError } = await supabase.from("customer_profiles").select("*").eq("id", user.customerId).maybeSingle();
  if (customerError) throw customerError;
  if (!customer) return { ok: false, status: 404, error: "Customer profile not found." };
  const [{ data: line }, { data: sim }, { data: cycle2 }, { data: usageRows }, { data: alertRows }, { data: blockedRows }] = await Promise.all([
  ...
  const usageEvents2 = (usageRows || []).map(usageEventFromRow);
  const usageGb = usageEvents2.reduce((sum, event) => sum + event.gbUsed, 0);
  const estimatedCharge = Math.min(usageGb * PRICE_PER_GB, MONTHLY_CAP);
  return {
    ok: true,
    data: {
      customer: customerFromRows(customer, line, sim, cycle2),
...
async function getCustomerDashboard(user) {
...
async function setPackieProtection(user, enabled) {
  const supabase = getSupabaseAdmin();
  if (supabase && user.customerId && isUuid(user.customerId)) {
    const { data, error } = await supabase.from("customer_profiles").update({ fraud_protection_status: enabled ? "enabled" : "disabled" }).eq("id", user.customerId).select("fraud_protection_status").maybeSingle();
    if (error) return { ok: false, status: 500, error: error.message };
    if (!data) return { ok: false, status: 404, error: "Customer profile not found." };
    return { ok: true, fraudProtectionStatus: data.fraud_protection_status };
  }
  const customer = customers.find((item) => item.id === user.customerId);
  if (!customer) return { ok: false, status: 404, error: "Customer profile not found." };
...
async function blockNumber(user, phoneNumber) {
  if (!user.customerId) return { ok: false, status: 403, error: "Customer access required." };
  const supabase = getSupabaseAdmin();
  if (supabase && isUuid(user.customerId)) {
    const { data: existing2, error: existingError } = await supabase.from("blocked_numbers").select("*").eq("customer_id", user.customerId).eq("phone_number", phoneNumber).maybeSingle();
    if (existingError) return { ok: false, status: 500, error: existingError.message };
    if (existing2) return { ok: true, blockedNumber: blockedNumberFromRow(existing2) };
    const { data, error } = await supabase.from("blocked_numbers").insert({ customer_id: user.customerId, phone_number: phoneNumber, reason: "Customer blocked number" }).select("*").single();
    if (error) return { ok: false, status: 500, error: error.message };
    return { ok: true, blockedNumber: blockedNumberFromRow(data) };
  }
...
async function unblockNumber(user, blockedNumberId) {
  const supabase = getSupabaseAdmin();
  if (supabase && user.customerId && isUuid(user.customerId)) {
    const { error, count } = await supabase.from("blocked_numbers").delete({ count: "exact" }).eq("id", blockedNumberId).eq("customer_id", user.customerId);
    if (error) return { ok: false, status: 500, error: error.message };
    if (!count) return { ok: false, status: 404, error: "Blocked number not found." };
    return { ok: true };
  }
